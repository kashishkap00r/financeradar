...
def parse_companies(payload, now=None):
    ...
    for item in _extract_items(payload):
        ...
        companies.append(
            {
                # The card is company + filing tag: the tag ("Acquisition",
                # "Dividend", "Buyback") carries the meaning far better than the
                # filing's own headline, which is mostly regulatory boilerplate.
                "title": company,
                "link": link,
                ...
                # The specific filing type. Not rendered — the `tag` chip is
                # what the card shows — but kept for search and data attributes.
